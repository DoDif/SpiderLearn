...
class BookPipeline(object):
    ...
    def open_spider(self, spider):
        self.db = MySQLdb.connect("localhost", "root", "1123", "books", charset="utf8")
...
class ChapterPipeline(object):
    ...
    def process_item(self, item, spider):
        print(item)
        FileAdd = "%s/book_chapter/%s.json" % (os.getcwd(), item["name"])
        print("start write" + FileAdd)
        with open(FileAdd.decode("utf-8"), "w") as file:
