...
class HybkSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for li in ul_list[0:2]:
            ...
            book_info["name"] = li.xpath("./li/a[@class='bn']/text()").extract_first().encode("utf-8")
            book_info["book_url"] = li.xpath("./li/a[@class='bn']/@href").extract_first().encode("utf-8")
            book_info["author"] = li.xpath("./li[@class='author']/text()").extract_first().encode("utf-8")
    ...
    def chapter_list(self, response):
        ...
        for chapter in chapter_li:
            book_info["chapter_info"]["chapter_name"] = chapter.xpath("./a/text()").extract_first().encode("utf-8")
            book_info["chapter_info"]["chapter_url"] = chapter.xpath("./a/@href").extract_first().encode("utf-8")
    ...
    def book_desc(self, response):
        book_info = response.meta["book"]
        book_info["type"] = response.xpath("//p[@class='info']/span/a/text()").extract_first().encode("utf-8")
        yield scrapy.Request(
